...
def foodHeuristic(state, problem):
    ...
    for coord in foodGrid.asList():
        # coord is where next food exists
        #distance = len(search.aStarSearch(problem))
        #man_distance = abs(position[0] - coord[0]) + abs(position[1] - coord[1])
        #euc_distance = ((position[0] - coord[0]) ** 2 + (position[1] - coord[1]) ** 2) ** 0.5
